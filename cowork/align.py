...
def mkd(args):
    try:
        os.mkdir('output_align')
    except:
        pass
    try:
        os.mkdir('output_align/' + args.codename.split('-')[0])
    except:
        pass
    try:
        os.mkdir('output_align/' + args.codename.split('-')[0]+'/'+args.codename)
    except:
        pass
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=6000, help='Number of training epochs Default: 100.')
    parser.add_argument('--batch-size', type=int, default=60, help='Input batch size for training. Default: 100.')
    ...
    parser.add_argument('--topology', type=str, default='FC_1000_FC_10', help='Choice of network topology. Format for convolutional layers: CONV_{output channels}_{kernel size}_{stride}_{padding}. Format for fully-connected layers: FC_{output units}.')
    ...
    train_align.train(args, device, train_loader, traintest_loader, test_loader)
