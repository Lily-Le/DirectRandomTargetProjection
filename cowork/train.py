...
# writer = SummaryWriter('logs/'+args.dataset+'/'+args.train-mode)
def train(args, device, train_loader, traintest_loader, test_loader):
    ...
    for trial in range(1,args.trials+1):
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            test_epoch(args, model, device, traintest_loader, loss, 'Train', epoch, writer)
            test_epoch(args, model, device, test_loader, loss, 'Test', epoch, writer)
...
def test_epoch(args, model, device, test_loader, loss, phase, epoch, writer):
    model.eval()
