...
class NetworkBuilder(nn.Module):
    ...
    """
    def __init__(self, topology, input_size, input_channels, label_features, train_batch_size, train_mode, dropout, conv_act, hidden_act, output_act, fc_zero_init,loss, device,tau=2.0,spike_window=100,surrogate_=surrogate.ATan()):
        ...
        for i in range(num_layers):
            ...
            try:
                if layer[0] == "CONV":
                    ...
                    input_dim    = input_size if (i==0) else int(output_dim/2) #/2 accounts for pooling operation of the previous convolutional layer
                    output_dim   = int((input_dim - int(layer[2]) + 2*int(layer[4]))/int(layer[3]))+1
                    self.layers.append(CSNN_block_VGG2(
                                       in_channels=in_channels,
                                       out_channels=int(layer[1]),
                                       kernel_size=int(layer[2]),
                                       stride=int(layer[3]),
                    ...
                    input_dim    = input_size if (i==0) else int(output_dim/2) #/2 accounts for pooling operation of the previous convolutional layer
                    output_dim   = int((input_dim - int(layer[2]) + 2*int(layer[4]))/int(layer[3]))+1
                    self.layers.append(CSNN_block_VGG3(
                                       in_channels=in_channels,
                                       out_channels=int(layer[1]),
                                       kernel_size=int(layer[2]),
                                       stride=int(layer[3]),
                ...
                elif layer[0] == "FCVS": # FC for VGG
                    if (i==0):
                        input_dim = pow(input_size,2)*input_channels
                        self.conv_to_fc = 0
                    elif (topology_layers[i-1][0]=="CONVS") or (topology_layers[i-1][0]=="CONV2S") or (topology_layers[i-1][0]=="CONV3S"):
                    ...
                    self.layers.append(FCS_block_VGG(
...
class CSNN_block_VGG2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias, tau,surrogate_function, dim_hook, label_features, train_mode):
        super(CSNN_block_VGG2, self).__init__()
        ...
        self.act1 = nn.Sequential(neuron.LIFNode(tau=tau,surrogate_function=surrogate_function))
        ...
        self.act2 = nn.Sequential(neuron.LIFNode(tau=tau,surrogate_function=surrogate_function))
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.hook = TrainingHook(label_features=label_features, dim_hook=dim_hook, train_mode=train_mode)
...
class CSNN_block_VGG3(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias, tau,surrogate_function, dim_hook, label_features, train_mode):
        super(CSNN_block_VGG3, self).__init__()
        ...
        self.act3 = nn.Sequential(neuron.LIFNode(tau=tau,surrogate_function=surrogate_function))
...
class FCS_block_VGG(nn.Module): # First Linear+Act, then Dropout
    def __init__(self, in_features, out_features, bias, tau,  dropout, dim_hook, label_features, fc_zero_init, train_mode):
        super(FCS_block_VGG, self).__init__()
